...
export const api = {
  ...
  simulatePayment: (body: {
    ...
    request<{ payment: PaymentRecord; resumed_instance_id: string | null }>(
      "/payments",
      {
        method: "POST",
  ...
  replayCase: (caseId: string, agent: "baseline" | "agent") =>
    request<EvalReplayResponse>(
      `/eval/fixtures/${encodeURIComponent(caseId)}/replay`,
      {
        method: "POST",
